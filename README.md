...
> FE·FastAPI 로컬 연결과 이번 계약 수정 내용: [docs/(BE)local-integration-guide.md](docs/%28BE%29local-integration-guide.md)
...
```bash
# 1. 개발 환경 셋업 — 컨테이너 + 스키마/시드 + 로컬 시크릿 생성
bash scripts/setup-local.sh

# 2. 빌드/실행 (JDK 21 필요, Linux/macOS/Git Bash 공통)
...
`setup-local.sh`는 기본 데모 시드와 함께 `scripts/seed-sample-100.sql`의 11번가 상품
100건도 멱등 적용한다. 원본 `../sample_100`이 변경되면 SQL을 다시 생성한다.
