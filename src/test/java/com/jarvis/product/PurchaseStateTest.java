...
class PurchaseStateTest {
    ...
    void derivesFromStatusAndStock() {
        assertThat(PurchaseState.of(product(ProductStatus.ON_SALE, 10))).isEqualTo(PurchaseState.AVAILABLE);
        assertThat(PurchaseState.of(product(ProductStatus.ON_SALE, 0))).isEqualTo(PurchaseState.SOLD_OUT);
    }
    ...
    void hiddenWinsOverSoldOut() {
        ...
        assertThat(PurchaseState.of(product(ProductStatus.HIDDEN, 0))).isEqualTo(PurchaseState.HIDDEN);
        assertThat(PurchaseState.of(product(ProductStatus.HIDDEN, 10))).isEqualTo(PurchaseState.HIDDEN);
    }
    ...
    void comparesAgainstRequestedQuantity() {
        ...
        assertThat(PurchaseState.of(product, 3)).isEqualTo(PurchaseState.AVAILABLE);
        assertThat(PurchaseState.of(product, 4)).isEqualTo(PurchaseState.SOLD_OUT);
    }

    @Test
    @DisplayName("목록·상세는 하나라도 살 수 있으면 AVAILABLE (수량 1 기준)")
    void listDefaultsToQuantityOne() {
        assertThat(PurchaseState.of(product(ProductStatus.ON_SALE, 1))).isEqualTo(PurchaseState.AVAILABLE);
    }

    private static Product product(ProductStatus status, int stockQuantity) {
