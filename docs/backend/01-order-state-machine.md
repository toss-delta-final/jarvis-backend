...
- `product_stock.quantity` 차감(2026-08-09 개정 — 구 `product.stock_quantity`, 02 D33)은 **결제 성공 처리(PENDING→PAID 전이, O-1·O-2 공통)와 같은 트랜잭션**에서 조건부 UPDATE로 수행한다(2026-07-17 확정). 차감 대상은 주문 줄이 담은 **옵션의 재고 행**이다 — 주문 생성 시점 차감이 아니므로 미결제(PENDING/PAYMENT_FAILED) 주문이 재고를 점유하지 않는다. 차감 실패(재고 부족) 시 결제 실패(`PAYMENT_FAILED`, reason `OUT_OF_STOCK`)로 처리. **2026-08-05 보완** — 이 경로만 두면 사용자가 재결제 무한 루프에 갇힌다(카드 거절과 응답이 같아 FE가 재결제를 유도하는데, 재고가 원인이면 절대 성공하지 않는다). 두 가지를 더했다: ① O-1은 **주문 생성 전에 재고를 선검증**해 부족하면 400 `ORDER_PRODUCT_UNAVAILABLE`(불량 품목 전량 수집 — 04 §4), ② 실패 사유를 응답 `failureReason`으로도 노출해 FE가 재결제/장바구니 동선을 가른다. **차감 시점 결정 자체는 그대로다** — 선검증은 UX 가드이고 이 조건부 UPDATE가 최종 방어선이며, 읽기와 차감 사이 경합은 여전히 여기로 떨어진다. O-2에는 선검증이 없어 그쪽 루프는 ②만으로 막는다. 취소/반품 완료 시 재고는 **아이템 종결 전이(`CANCELLED`/`RETURNED`)와 같은 트랜잭션에서 복원**한다(2026-08-10 도입 — 구 "MVP 미구현, 감수"). 신청(`*_REQUESTED`)이 아니라 **승인 시점**인 이유는 신청이 아직 확정이 아니라서다 — 미리 풀면 팔 수 없는 물건을 파는 셈이 된다. 복원 키는 `order_item.option_id`(D33 개정으로 추가 — `option_name`은 스냅샷이라 현재 옵션과 매칭이 보장되지 않는다). 재고가 **0에서 풀리는 경우에만** STOCK 로그 1행(`old_value=0`) — 차감의 품절 로그와 대칭이며, 그 밖의 복원은 주문 유래라 미기록이다(02 D32). 재고 차감 자체는 order_status_logs 기록 대상이 아니다(product_change_logs 규칙 — 02 소관).
