...
review(M-1·M-3 — **M-2는 MVP 제외**, FE 화면 없음 07-17) + P-3 실데이터(별점 분포 distribution 포함), wishlist(M-4~6), recent(M-7), address(M-8). (**M-10 프로필 수정도 MVP 제외** — 07-17) (관리자 AD-1~7은 MVP 제외 — 2026-07-09 팀 결정, 04 §9) **M-9 문의 내역은 2026-08-07 폐기** — 문의 기능 전체를 걷어냈다
- **완료**: 후기 자격 상태(DELIVERED/CONFIRMED — 교환 제거, 01 D11)에서만 작성됨(그 외 400), 후기 신고 접수·중복 신고 409 확인. 신고 처리(HIDE)는 고도화. 문의 답변(AD-1·AD-2)은 2026-08-07 폐기.

## Phase 5. 채팅 티켓 발급 + 카드 하이드레이션 + internal API (1.5일, LLM팀 병행 필요)

세션 발급(Redis TTL), **CH-1 세션+스트림 티켓(RS256) 발급 + JWKS 엔드포인트(`/.well-known/jwks.json`)**, **추천 목록 콜백(I-21) + 목록 조회(CH-5)**, CH-1b 티켓 재발급, 게스트 쿠키 발급, internal(04 §10 중 확정분 — I-13·I-17·I-21 OPEN은 스텁) + 서비스 토큰 필터. 채팅 SSE는 FastAPI 직결이라 **Spring은 SSE를 중계하지 않는다**(03 D5) — BE는 티켓 발급·검증키·콜백만 책임. FastAPI가 아직 없으면 **mock FastAPI**(고정 SSE 반환·티켓 JWKS 검증 스텁)로 FE 직결 흐름을 먼저 검증.
...
- **(2026-07-18 구현 확정)** CH-5는 05 제안 스키마대로 임시 구현(`{listId, items[카드]}` — 순서 보존·HIDDEN/품절 드롭), I-21은 `{sessionId, listId, productIds[]}`(UUID 필수·≤20·Redis TTL 10분). **(2026-07-30 개정 — I-21 구현 완료)** I-21은 다중 `lists[]`+`listType`+**목록당 9개**·목록 10개 상한을 받고 Redis(조회 전용)+DB(정본, D38) 양쪽에 저장한다. ~~FastAPI가 전환하기 전까지 **구 평평 구조(`listId`·`productIds`)도 함께 수용**한다 — 양쪽 배포 시점을 묶지 않으려는 과도기 조치이므로 전환 후 제거한다.~~ **(2026-08-07 제거 완료)** LLM 팀 전환이 끝나(jarvis-ai v0.17.1, 이슈 #209 — 그쪽 스키마도 `lists`·`listType`·`recommendationRequestId`를 전부 필수로 선언) 과도기 수용 코드를 걷어냈다. 함께 `listType`·`recommendationRequestId`의 기본값 채우기도 폐지해 누락은 400이다(노션 I-21 실패 응답 표 개정). **CH-5 확장도 구현 완료(2026-07-30)** — `recommendationRequestId`·`listType`(항상)·`label?`·BUY_ALL이면 `totalBudget`·`sum`(드롭 후 재계산)·`withinBudget`(판정 불가 시 null 리터럴). **`recommendation_generated` 서버 적재 완료**(목록당 1행·재전송분 제외, `session_key`는 채팅 세션 id — 02 D39). **E-1 확장도 완료(2026-07-30)** — FE 추천 4종(`recommendation_impression`·`product_visible`·`product_click`·`recommendation_dismiss`) 수용, `occurredAt`을 `occurred_at`에 저장하고 이상치(미래·3일 초과 과거·누락)는 수신 시각으로 대체 + `_timeShifted`, 추천 귀속은 `list_id`로 DB 사본을 조회해 **서버가 도출**(FE 값 불신 — ③.5), 필수 `properties` 키 누락 시 `_incomplete`, `schemaVersion`은 전용 컬럼 없이 `properties`에. `occurred_at`·`client_event_id` **NOT NULL 전환 완료**(2026-07-30) — 기존 행 백필이 필요해 별도 마이그레이션(`scripts/migrate-2026-07-31-...`)으로 분리했고, **이 확장이 배포된 뒤에** 적용한다(순서를 뒤집으면 그 사이 이벤트가 유실된다 — DEPLOY.md §4-2). 서버 적재분의 `client_event_id`는 `listId` 기반 결정적 UUID(02 D40). → **추천 축(I-21·CH-5·E-1) 구현 완결**. I-20 통지는 로그아웃 트리거만(2026-07-30 개정 — 새 대화 사유 소멸) — 30분 유휴는 Redis TTL 자연 소멸이라 통지 없음(FastAPI 자체 TTL이 백스톱, 05 §3).
