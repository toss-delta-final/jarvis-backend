# 05. LLM(FastAPI) 연동 계약 — v0.4 (2026-07-21 · 노션 정본 정합)
...
- 트리거(실발화): 로그아웃(`LOGOUT`) / 새 대화·가입·로그인 승계(`NEW_CONVERSATION`). body: `{ "sessionId": "<uuid>", "reason": "LOGOUT" }` — reason enum `LOGOUT|IDLE_TIMEOUT|NEW_CONVERSATION|TAB_CLOSE`(뒤 2종은 예약 — 유휴·탭 종료는 통지 없이 TTL 소멸, FastAPI는 수신을 전제하지 말 것).
