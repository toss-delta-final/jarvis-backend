...
| I-11 | PATCH | /internal/seller/{brandId}/products/{productId} | 상품 수정 통합(가격·설명·상태·재고) — 바뀐 필드마다 product_change_logs 기록(동일값 미기록), 응답에 changes[]. HITL confirm 후 실행(05 §1-3) — 상품 수정의 유일 경로(구 S-5 직접수정 폐기 2026-07-21). **삭제된 상품은 409 `PRODUCT_DELETED`**, `status=DELETED` 지정도 거부 — 삭제는 I-12 전용 전이(02 D41) |
