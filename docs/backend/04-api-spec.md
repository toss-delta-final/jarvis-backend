...
| # | Method | 경로 | 인증 | 설명 |
|---|---|---|---|---|
| CH-1 | POST | /api/chat/sessions | 🔓(게스트 허용) | 세션 발급(Redis TTL 10분) **+ 스트림 티켓(RS256/JWKS) 동시 발급**. body: `channel`(SHOPPING\|CS). 신원 확인(회원 JWT/게스트 쿠키) 후 `sub/sub_type/scope:chat:stream` 티켓 서명. 응답: `sessionId, ttlSeconds(600), streamTicket, ticketTtlSeconds(30~60s), llmSseUrl`. **멱등 발급**(노션 CH-1 2026-07-31 개정 · SPEC-CHAT-SESSION D5) — 같은 신원·채널의 활성 세션이 있으면 축출 없이 **그대로 반환**(owner 키 SETNX) + 세션·owner TTL sliding. 세션은 **신원+채널 단위로 공존**(SHOPPING·CS·SELLER가 서로를 밀어내지 않음)하며, "새 대화"는 FE가 `threadId`만 갱신해 이 API를 부르지 않는다. **실패: 400 `VALIDATION_ERROR`**(잘못된 값 — `channel`이 SHOPPING\|CS 밖이거나 body 파싱 불가. `SELLER`도 여기 해당, 입구는 S-4뿐) (노션 CH-1 실패 응답표 2026-07-28) |
...
| I-20 | POST | {LLM_BASE_URL}/events/session-end | **방향 예외: Spring→FastAPI(아웃바운드)** 세션 종료 통지 — 트리거는 **로그아웃 하나뿐**(회원만, 노션 I-20 정본 2026-07-30), reason enum LOGOUT, **멱등**(응답 202 {status: accepted\|duplicate}). sessionId는 **UUID 그대로 수신(2026-07-17 LLM 합의 — 구 S- 형식 폐기)**. 상세는 05 §2-1(아웃바운드 관례) |
